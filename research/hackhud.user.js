...
extend(UserScript, {
	...
	metaUrl: 'http://dharmafly.com/projects/hackhud',
...
function gEBCC(tagName, cssClass){
	// Construct empty array to return
	var returnArray = [];
	
	// Get divs in the document
	var elements = gEBTN(tagName);
...
function getArticleTitle(){
	// Get the div with class name 'sh'
	var titleEl = gEBCC('div', 'sh')[0];
	return getTextValue(titleEl);
}


function getArticleNode(){
	// Get <td> elements - we want to find the one that contains the main text
	var tds = gEBTN('td');	
	
	// Work out how many <p> elements within each <td> - create an array to store each <td> element and the number of <p> elements it contains... Best way we could think of to fight through the jungle of markup on the BBC site...
	var pMap = [];
	for (var i=0; i<tds.length; i++){
		var td = tds[i];
		pMap.push({
			node: td,
			pCount: td.getElementsByTagName('p').length
		});
	}
	
	// Sort pMap array, with highest number of <p>'s first
	pMap.sort(function sortNumbers(a, b){
		return b.pCount - a.pCount;
	});
	
	
	// The first <td> is an outer-container for the content plus other gumph... The inner <td> is the one we want...
	return pMap[1].node;
}
...
function getArticleContent(){
	var node = getArticleNode();
	
	// Find <b> tags! BBC use them to mark up the leader paragraph of each article, as well as each sub-heading within the article. Ah... the semantic web!
	var bolds = node.getElementsByTagName('b');
	
	// Take the first <b> to be the leader summary paragraph...
	var leader = getTextValue(bolds[0]);
	
	// Now take the rest to be sub-headings - these are still maintained in the 'content' variable
	var subheadings = [];	
	for (var i=1; i<bolds.length; i++){
		subheadings.push(bolds[i]);
	}
	
	// The BBC puts all the content into <font> tags... Great! And who said that presentational markup was dead?
	var fonts = node.getElementsByTagName('font');
	var content = '';
	for (var i=0;fonts[i];i++){
		var c = fonts[i].firstChild.nodeValue;
	    content += c !== null ? c : '';
	}
...
function addMenuListeners() {
	...
	for (var i=0;i<aHackTags.length;i++) {
        ...
        oMenu.addItems([
			{ text:"Flickr", onclick: {fn:paramCall(addContentToPanel, this, ["Flickr", getFlickrPipesData])} },
			{ text:"Technorati", onclick: {fn:paramCall(addContentToPanel, this, ["Technorati", getTechnoratiPipesData])} },
			{ text:"Newsvine", onclick: {fn:paramCall(addContentToPanel, this, ["Newsvine", getNewsvinePipesData])} }
		]);
...
function createPanel(){
	...
	YAHOO.HUD.infoPanel.setFooter(UserScript.title);
...
function ajaxRequest(sUrl, fCallback){
	GM_xmlhttpRequest({
		...
		onload: fCallback,
		onerror: function(){ fCallback(false); }
	});
}


function getYahooPipe(sId, sTags, nNumber, fCallback) {
	sUrl = 'http://pipes.yahoo.com/pipes/pipe.run?_cmd=Run+Pipe&number=' + (nNumber||4) + '&_id=' + sId + '&_render=json&_run=1&keywords=' + sTags;
...
function getFlickrPipesData(sTags, oInfoPanel) {
	getYahooPipe('_C5GAmQc3BGB1RZYJxOy0Q', sTags, 4, 
...
function checkGreasemonkeyVersion(){
	...
	if (!GM_xmlhttpRequest) {
		if (window.confirm('You do not have the latest version of Greasemonkey installed.\nWould you like to get it now?')){
			window.location.href = 'http://www.greasespot.net';
		}
		return false;
	}
